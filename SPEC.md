...
- **Service**: business logic, owns transaction qua `transaction()` helper. Throw `AppException`, không biết gì về HTTP.
- **Repository**: tất cả DB queries. Abstract Supabase/SQLAlchemy — nếu đổi DB chỉ sửa repo.
...
async def get_db():
    async with AsyncSession(engine) as session:
        yield session  # không begin, không manage transaction

# items/service.py — service owns transaction boundary
class ItemService:
...
- `get_db` chỉ tạo và yield session; cleanup do `session.close()` + pool `reset_on_return` xử lý
- Committed data không bị ảnh hưởng nếu exception xảy ra sau commit
...
- Status codes dùng `status.HTTP_201_CREATED`, `status.HTTP_204_NO_CONTENT`, v.v.
- Tất cả endpoints khai báo `response_model`
- Duplicate check: pre-check trong service + DB unique constraint làm safety net
...
**AI processing pipeline — non-blocking:**
- Upload → trả `202 Accepted` ngay
- rembg + Gemini tagging chạy trên **ARQ worker** (process riêng) ngay từ MVP — không dùng FastAPI `BackgroundTasks` vì rembg load model nặng, Gemini call blocking, dễ làm chết API process
- Frontend dùng Supabase Realtime để nhận update khi processing xong
- Mọi bước processing update `processing_status`; lỗi phải lưu `processing_error` để UI có thể retry

**Các quy tắc khác:**
- Tất cả I/O phải `async/await` — không mix sync trong async FastAPI
- Rate limiting trên AI endpoints (slowapi) — bảo vệ Gemini quota
- Structured logging (loguru) — log `request_id`, `user_id`, AI call duration
- Tất cả AI prompts trong file `prompts.py` riêng của mỗi feature
- Gemini tagging prompt **phải truyền taxonomy enum values** — không để AI tự sáng tác type/style/occasion ngoài danh sách
- Supabase RLS bắt buộc — user chỉ read/write data của mình
- External services được wrap bằng interface/client riêng để test dễ mock: `GeminiClient`, `WeatherClient`, `StorageClient`, `BackgroundRemovalClient`
- Không gửi toàn bộ ảnh full-size vào Gemini nếu thumbnail/processed image đủ cho tagging
- Khi log wear (`POST /outfits/{id}/wear`): service phải fetch và snapshot toàn bộ item data trước khi insert `wear_logs`
